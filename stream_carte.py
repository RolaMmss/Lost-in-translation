...
# # Récupération des données de la table 'objet_perdu_annee' pour l'année et le type d'objet sélectionnés
# if objet_type == "Tous":
#     query = f"SELECT * FROM gares_objets_perdus WHERE annee = {year}"
# else:
#     query = f"SELECT * FROM gares_objets_perdus WHERE annee = {year} AND gare IN (SELECT DISTINCT gare FROM objets_trouves WHERE type_objet = '{objet_type}')"
...
if objet_type == "Tous":
    query = f"SELECT * FROM objets_trouves WHERE date LIKE '{year}-%'"
else:
    query = f"SELECT * FROM objets_trouves WHERE date LIKE '{year}-%' AND type_objet = '{objet_type}'"
